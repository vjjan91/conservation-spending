...
```{r}
## Category Lists
clim.energy_cat <- names(data)[c(37,38)]   # threat_climate and threat_energytransport

bio.filter_cat <- names(data)[c(19:28, 30:36)]

# Grants that were classified into a single category only
data_bio.1 <- data %>%
  filter_at(vars(bio.filter_cat), any_vars(.> 0)) 
data_non.bio.1 <- data %>%
  filter_at(vars(bio.filter_cat), all_vars(. == 0))
...
In the "data_multi" dataset, grants classified into multiple sub-categories (e.g. threat_climate + threat_habitat) would be over-represented in our analysis of funding. We sought to only count the amount of funding for a grant once within each category. Otherwise,  To accomplish this, we modified the dataset in several ways:
  1. Grants classified into two sub-categories were classified into a new column combining the two (e.g. threat_climate_habitat)
  2. Grants classified into three or more sub-categories were classified into new "other" column (e.g. threat_other)
  3. The habitat category had joint sub-categories in the initial list of categories (e.g. hab_marine_terrestrial). Any grants classified into both a single (e.g. hab_marine) and an overlapping joint sub-category (e.g. hab_marine_terrestrial) were classified as only the joint (e.g. hab_marine_terrestrial)
