...
```{r}
funders_top500 <- read_csv("data/data-list-of-top500-donors-by-money.csv")
...
fig_top_funders <- ggplot(top_funders, aes(x = n_funders,
  ...
  geom_line() +
  geom_point() +
  geom_area(aes(y=ifelse(tot_money_mil< top_funders[6,2], tot_money_mil, top_funders[6,2])),
            fill="#696969", alpha = .4) +
  theme_minimal() +
...
# % Money captured by top 400 funders = 98%
top400 <- money[6]/money[7]
top400

ggsave(fig_top_funders, filename = "figs\\fig_topGrantmakers_funding.png", 
       height = 8, width = 11, device = png(), units="in", dpi = 300)

```

```{r ,eval=TRUE, fig.cap="Accumulation curve of the funding in our dataset across the top 1000 grantmakers. The shaded region encompasses the amount of funding captured by the top 400 grantmakers, which represents more than 98% of the total amount of funding."}
# show exported image
